...
adminpem="/etc/opensearch/certs/admin.pem"
...
function passwords_changePassword() {
    ...
    if [ -n "${changeall}" ]; then
        if [ -n "${indexer_installed}" ] && [ -z ${no_indexer_backup} ]; then
            eval "mkdir /etc/opensearch/backup/ ${debug}"
            eval "cp /etc/opensearch/opensearch-security/* /etc/opensearch/backup/ ${debug}"
            passwords_createBackUp
        fi
        for i in "${!passwords[@]}"
        do
            if [ -n "${indexer_installed}" ] && [ -f "/etc/opensearch/backup/internal_users.yml" ]; then
                awk -v new='"'"${hashes[i]}"'"' 'prev=="'${users[i]}':"{sub(/\042.*/,""); $0=$0 new} {prev=$1} 1' /etc/opensearch/backup/internal_users.yml > internal_users.yml_tmp && mv -f internal_users.yml_tmp /etc/opensearch/backup/internal_users.yml
            fi
        ...
        if [ -n "${indexer_installed}" ] && [ -f "/etc/opensearch/backup/internal_users.yml" ]; then
            awk -v new='"'"${hash}"'"' 'prev=="'${nuser}':"{sub(/\042.*/,""); $0=$0 new} {prev=$1} 1' /etc/opensearch/backup/internal_users.yml > internal_users.yml_tmp && mv -f internal_users.yml_tmp /etc/opensearch/backup/internal_users.yml
        fi
    ...
    if [ "$nuser" == "kibanaserver" ] || [ -n "$changeall" ]; then
        if [ -n "${dashboard_installed}" ] && [ -n "${dashpass}" ]; then
            if /usr/share/opensearch-dashboards/bin/opensearch-dashboards-keystore --allow-root list | grep -q opensearch.password; then
                eval "echo ${dashpass} | /usr/share/opensearch-dashboards/bin/opensearch-dashboards-keystore --allow-root add -f --stdin opensearch.password ${debug_pass} > /dev/null 2>&1"
            else
                wazuhdashold=$(grep "password:" /etc/opensearch-dashboards/opensearch_dashboards.yml )
                rk="opensearch.password: "
                wazuhdashold="${wazuhdashold//$rk}"
                conf="$(awk '{sub("opensearch.password: .*", "opensearch.password: '"${dashpass}"'")}1' /etc/opensearch-dashboards/opensearch_dashboards.yml)"
                echo "${conf}" > /etc/opensearch-dashboards/opensearch_dashboards.yml
            fi
            passwords_restartService "opensearch-dashboards"
        fi
    fi
...
function passwords_changeDashboardApiPassword() {
    ...
    until [ -n "${file_exists}" ] || [ "${j}" -eq "12" ]; do
        if [ -f "/usr/share/opensearch-dashboards/data/wazuh/config/wazuh.yml" ]; then
            eval "sed -i 's|password: .*|password: \"${1}\"|g' /usr/share/opensearch-dashboards/data/wazuh/config/wazuh.yml ${debug}"
            if [ -z "${AIO}" ] && [ -z "${indexer}" ] && [ -z "${dashboard}" ] && [ -z "${wazuh}" ] && [ -z "${start_indexer_cluster}" ]; then
...
function passwords_createBackUp() {
    ...
    if [ -z "${indexer_installed}" ] && [ -z "${dashboard_installed}" ] && [ -z "${filebeat_installed}" ]; then
        ...
        if [ -n "${indexer_installed}" ]; then
            capem=$(grep "plugins.security.ssl.transport.pemtrustedcas_filepath: " /etc/opensearch/opensearch.yml )
    ...
    if [ ! -d "/etc/opensearch/backup" ]; then
        eval "mkdir /etc/opensearch/backup ${debug}"
    fi
    eval "JAVA_HOME=/usr/share/opensearch/jdk/ OPENSEARCH_CONF_DIR=/etc/opensearch /usr/share/opensearch/plugins/opensearch-security/tools/securityadmin.sh -backup /etc/opensearch/backup -icl -p 9200 -nhnv -cacert ${capem} -cert ${adminpem} -key ${adminkey} -h ${IP} ${debug}"
    ...
    common_logger -d "Password backup created in /etc/opensearch/backup."
...
function passwords_generateHash() {
    ...
    if [ -n "${changeall}" ]; then
        ...
        for i in "${!passwords[@]}"
        do
            nhash=$(bash /usr/share/opensearch/plugins/opensearch-security/tools/hash.sh -p "${passwords[i]}" 2>&1 | grep -A 2 'issues' | tail -n 1)
            if [  "${PIPESTATUS[0]}" != 0  ]; then
        ...
        hash=$(bash /usr/share/opensearch/plugins/opensearch-security/tools/hash.sh -p "${password}" 2>&1 | grep -A 2 'issues' | tail -n 1)
        if [  "${PIPESTATUS[0]}" != 0  ]; then
            common_logger -e "Hash generation failed."
            if [[ $(type -t installCommon_rollBack) == "function" ]]; then
                installCommon_rollBack
...
function passwords_getNetworkHost() {
    ...
    IP=$(grep -hr "^network.host:" /etc/opensearch/opensearch.yml)
    NH="network.host: "
    IP="${IP//$NH}"
...
function passwords_readUsers() {
    ...
    susers=$(grep -B 1 hash: /etc/opensearch/opensearch-security/internal_users.yml | grep -v hash: | grep -v "-" | awk '{ print substr( $0, 1, length($0)-1 ) }')
...
function passwords_runSecurityAdmin() {
    ...
    eval "OPENSEARCH_CONF_DIR=/etc/opensearch /usr/share/opensearch/plugins/opensearch-security/tools/securityadmin.sh -f /etc/opensearch/backup/internal_users.yml -t internalusers -p 9200 -nhnv -cacert ${capem} -cert ${adminpem} -key ${adminkey} -icl -h ${IP} ${debug}"
    if [  "${PIPESTATUS[0]}" != 0  ]; then
        common_logger -e "Could not load the changes."
        exit 1;
    fi
    eval "cp /etc/opensearch/backup/internal_users.yml /etc/opensearch/opensearch-security/internal_users.yml"
    eval "rm -rf /etc/opensearch/backup/ ${debug}"
...
function passwords_updateInternalUsers() {
    ...
    internal_users_backup_path="/etc/opensearch/internalusers-backup"
    passwords_getNetworkHost
    passwords_createBackUp

    eval "mkdir -p ${internal_users_backup_path} ${debug}"
    eval "cp /etc/opensearch/backup/internal_users.yml ${internal_users_backup_path}/internal_users_${backup_datetime}.yml.bkp ${debug}"
    ...
    eval "chown -R opensearch:opensearch ${internal_users_backup_path} ${debug}"
    common_logger "A backup of the internal users has been saved in the /etc/opensearch/internalusers-backup folder."

    eval "cp /etc/opensearch/backup/internal_users.yml /etc/opensearch/opensearch-security/internal_users.yml ${debug}"
    eval "rm -rf /etc/opensearch/backup/ ${debug}"
...
function common_checkInstalled() {
    ...
    if [ "${sys_type}" == "yum" ]; then
        eval "rpm -q opensearch --quiet && indexer_installed=1"

    elif [ "${sys_type}" == "apt-get" ]; then
        indexer_installed=$(apt list --installed 2>/dev/null | grep opensearch)
    fi
    ...
    if [ "${sys_type}" == "yum" ]; then
        eval "rpm -q opensearch-dashboards --quiet && dashboard_installed=1"
    elif [ "${sys_type}" == "apt-get" ]; then
        dashboard_installed=$(apt list --installed  2>/dev/null | grep opensearch-dashboards)
    fi

    if [ -d "/var/lib/opensearch-dashboards/" ] || [ -d "/usr/share/opensearch-dashboards" ] || [ -d "/etc/opensearch-dashboards" ] || [ -d "/run/opensearch-dashboards/" ]; then
